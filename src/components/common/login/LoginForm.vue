...
<script>
import FormTem from "./FormTem";
import CheckBox from "./CheckBox";
import Captcha from "components/common/captcha/Captcha";
import { postRegisterUser, postLoginUser } from "network/login";
import { CHANGE_UID } from "store/mutation-types";
import { TK, R_TK, getUid } from "common/const";
export default {
  data() {
    return {
      isKeep: false,
  ...
  methods: {
    ...
    checkLogin() {
      ...
      if (this.isLoginTrue) {
        ...
        this.$toast.toastShow("请输入账号密码", 2000);
      }
    },
    //检测注册用函数
    checkRegister() {
      ...
      if (this.isRegisterTrue) {
        ...
        this.$toast.toastShow("请正确填写信息", 2000);
      }
    },
    /**
     * 网络相关
    ...
    async postRegisterUser() {
      ...
      if (!res.success) {
        ...
        this.$toast.toastShow("注册成功", 1500);
      }
    },
    //登录
    async postLoginUser() {
      ...
      if (res) {
        this.$toast.toastShow("登录成功", 1500);
